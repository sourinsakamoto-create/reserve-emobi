...
import { updateActivityAction, toggleActivitySaleAction, deleteActivityAction } from "./actions";
  ...
  const bookingCounts = await Promise.all(
    activities.map((activity) =>
      prisma.booking.count({
        where: { scheduleSlot: { activityId: activity.id }, status: "CONFIRMED" },
      })
    )
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <p className="text-sm text-neutral-600">
        まず一覧で「販売中/停止」の切り替えや削除ができます。内容(説明文・料金など)を編集したいプランは「詳細設定」を開いてください。
      </p>

      <ul className="space-y-3">
        {activities.map((activity, i) => {
          const bookedCount = bookingCounts[i];
          return (
            <li key={activity.id} className="border border-neutral-200 rounded-lg bg-white overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-3 p-4">
                <div>
                  ...
                  {bookedCount > 0 && (
                    <p className="text-xs text-neutral-500 mt-1">現在の確定予約: {bookedCount}件</p>
                  )}
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <form action={deleteActivityAction}>
                    ...
                    <ConfirmSubmitButton
                      ...
                      confirmMessage={
                        bookedCount > 0
                          ? `このアクティビティには現在${bookedCount}件の確定予約があります。削除するとそれらの予約はキャンセル扱いとなり、お客様(および担当ガイド)にキャンセルのメールが送信されます。よろしいですか？`
                          : "このアクティビティを削除します。よろしいですか？"
                      }
                    >
                      削除
                    </ConfirmSubmitButton>
              ...
              <details className="border-t border-neutral-200">
                <summary className="cursor-pointer select-none px-4 py-2 text-sm text-emerald-700 hover:bg-neutral-50">
                  詳細設定を開く(説明文・料金・画像などを編集)
                </summary>
                <form
                  action={updateActivityAction}
                  className="grid gap-3 sm:grid-cols-2 p-4 pt-2"
                >
                  <input type="hidden" name="id" value={activity.id} />
