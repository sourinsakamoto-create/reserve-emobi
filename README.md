...
- **日程の一覧確認・運行可否の監視**: `/admin/schedule` は「リスト表示」(全件の表)と「カレンダー表示」(月表示、日ごとの便数とガイド配置状況が一目でわかる)を切り替えられます。`/admin/schedule/list` では全アクティビティ横断の一覧も見られます。「枠が設定されている(在庫がある)こと」と「実際にガイドが配置されていて運行できること」は別物のため、各枠に「運行可否」列(ガイド配置済み件数 / 未配置)を表示し、`/admin/schedule/list` では「ガイド未配置のみ表示」フィルタでガイドの配置漏れを監視できます
- **必要車両台数の自動算出**: お客様3名につきトゥクトゥク1台、それに加えてガイド用に1台が必要という前提で、便ごとの必要車両台数(お客様用+ガイド用)を自動計算します。`/admin/schedule` と `/admin/schedule/list` の「必要車両」列で確認できるほか、新規予約時の管理者通知メールにも記載されます
- **画像アップロード**: アクティビティごとに、管理画面から直接画像ファイルをアップロードできます(Vercel Blobを使用。未設定の場合は画像URLを直接指定する方法にフォールバックします)
- **ガイドの出勤可能日管理**: ガイド役割のスタッフが `/guide` から、日付ごとの各便(アクティビティ×時間)単位で「担当可能」をチェックし、「保存する」を押した時点でまとめて反映されます(押すまでは画面上の見た目が変わるだけで保存されません)。保存すると管理者通知アドレスにも更新内容が届きます。管理者は `/admin/guides` で、便(行)×ガイド(列)のマトリクス表で全ガイドの登録状況を横断的に確認できます。表は「日時順」「プラン順」「ガイド未配置を優先」で並び替えられ、この登録状況に基づいてお客様の予約可否を制限するかどうかをON/OFFできます(デフォルトはOFF)
...
担当ガイドへの通知メールの宛名は、そのガイドアカウントの `StaffUser.name`(名前)をそのまま使用します。`npm run db:seed` で作成される最初の管理者アカウントは名前が「管理者」固定になっているため、そのアカウントで `/guide` から出勤登録も行っている場合はメールの宛名が「管理者様」になります。実際の担当者名で届くようにしたい場合は、`/admin/staff` からそのアカウントの名前を実名に変更してください(すでに編集・削除に対応しています)。
