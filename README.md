...
- **ガイドの出勤可能日管理**: ガイド役割のスタッフが `/guide` から、日付ごとの各便(アクティビティ×時間)単位で「担当可能」をチェックし、「保存する」を押した時点でまとめて反映されます(押すまでは画面上の見た目が変わるだけで保存されません)。保存すると管理者通知アドレスにも更新内容が届きます。管理者は `/admin/guides` で、便(行)×ガイド(列)のマトリクス表で全ガイドの登録状況を横断的に確認できるほか、この登録状況に基づいてお客様の予約可否を制限するかどうかをON/OFFできます(デフォルトはOFF)
